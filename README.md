...
- [ ] **Re-arm auto-login after imaging** — a VM created from a built image boots to the login screen (password `admin`) on its first boot; macOS doesn't honor the `kcpassword` auto-login until one interactive login "arms" it on a restored system. The vsock agent (a root launchd daemon) works regardless, so this only matters for GUI/UI-test CI. Investigate `sysadminctl`/SecureToken or a first-boot LaunchDaemon that completes the login.
