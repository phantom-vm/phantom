...
- [ ] **1. Headless install** — verify `vm.create` installs macOS from IPSW without GUI interaction; support `ipsw pull latest` via `VZMacOSRestoreImage.fetchLatestSupported()`
- [ ] **2. VNC server in daemon** — start `_VZVNCServer` (private API, invoked via reflection) for a running VM; it works during installation and Setup Assistant with no guest-side software. New API: `vm.vnc` → `vnc://:password@127.0.0.1:port`
