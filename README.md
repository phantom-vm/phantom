...
- [x] **Flush guest disk before saving an image** — `vm.stop` is a force stop (`VZVirtualMachine.stop()` = "like unplugging the machine"), so writes still in the guest's buffer cache were lost from the saved image. This silently dropped the agent install, provisioning, *and* the `kcpassword` file (the earlier "auto-login needs re-arming after imaging" was a misdiagnosis of this same bug). Fix: the orchestrator runs `sync` in the guest before stopping. Verified: a VM created from the image now has the agent running, passwordless sudo for `admin`, and working auto-login.
