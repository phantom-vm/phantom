...
**Interactive exec** — `phantom vm exec -it <id> -- zsh` puts the local terminal in raw mode and wires it to a pty in the guest: keystrokes go up as `stdin` frames, `SIGWINCH` as `resize` frames, and output comes back as base64 chunks written straight through. Every exit path restores the terminal, including the one where the daemon closes without a final chunk — a shell left in raw mode is worse than a wrong exit code. Before starting, it probes the guest with a one-command tty session (`test -t 0`): an image whose agent predates this ignores the flag and would otherwise hand back a terminal that quietly is not one, and the probe turns that into a sentence explaining what to rebuild.
