...
- **Sizing and naming**: `name` becomes the VM's id and its bundle directory, so it is restricted to letters, digits, `-` and `_` and rejected if a VM already has it; omitted, a `vm-xxxxxxxx` is generated. `cpuCount` and `memoryGB` are range-checked against the host (the framework's CPU ceiling capped at the machine's core count, memory at installed RAM) and stored in the bundle's `vm.json`, so they apply to every later start rather than only the first. Passing one leaves the other at its default, and the default is derived from the host — half its cores, a quarter of its memory, floored at 8GB — not a fixed 4/16GB. A clone without them inherits the source VM's sizing.
- **Implementation**:
...
- **Implementation**: Connects to guest agent on vsock port 9001. When `waitForAgent` is true, retries connection every 2s for up to 120s (for freshly booted VMs). The agent runs as root, and the daemon wraps the command in `su - <user> -c '…'` so it runs as that user with their login environment (no agent change needed). With an auto-logged-in user this even reaches the GUI session — e.g. `-- open -a Notes` opens Notes on the desktop.
- **Who it runs as**: `user` defaults to **`admin`**, not root. Root's PATH comes from launchd — `/usr/bin:/bin:/usr/sbin:/sbin` — with no `/usr/local/bin`, no `~/.local/bin` and no mise shims, so tools an image was built to provide (`gitlab-runner`, `mise`, `bun`) are invisible to it while working perfectly for the CI jobs that run as admin. A command that shows something other than what a job sees is worst exactly when it is reached for, which is when something looks wrong. Pass `user: "root"` for the privileged case; provisioning does, since it is what creates admin's passwordless sudo and so cannot lean on it.
